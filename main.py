...
small_font = pygame.font.Font(None, 18) # Reduzido de 20 para 18
medium_font = pygame.font.Font(None, 24) # Reduzido de 30 para 24
...
class Game:
    ...
    def is_position_safe(self, pos, board, current_player_char):
        """
        Verifica se uma dada posição (pos) no tabuleiro é segura para a peça
        pertencente a current_player_char. Uma posição é insegura se uma peça
        do oponente puder capturá-la em seu próximo turno.
        """
        row, col = pos
    ...
    def draw(self):
        """Desenha o tabuleiro do jogo, peças, destaques e informações do jogo."""
        # Desenha o tabuleiro com quadrados pretos e brancos
        for row in range(8):
            for col in range(8):
        ...
        if self.status != 'Game Over':
            ...
            # Exibe o turno atual
            if self.turn % 2 == 0:
                turn_text = medium_font.render("Turno do Azul", True, BLUE)
            else:
                turn_text = medium_font.render("Turno do Rosa", True, PINK)
...
def show_winner(winner):
    ...
    while running:
        ...
        if winner == "tie":
            text_surf, text_rect = text_objects("EMPATE!", large_font, WHITE)
        elif winner == "x":
            text_surf, text_rect = text_objects("ROSA GANHOU!", large_font, PINK)
        else: # winner == "o"
            text_surf, text_rect = text_objects("AZUL GANHOU!", large_font, BLUE)
...
def main_menu():
    ...
    # Definindo os retângulos dos botões UMA VEZ
    play_rect = pygame.Rect(WIDTH // 2 - 100, HEIGHT // 2 - 30, 200, 50)
    play_comp_rect = pygame.Rect(WIDTH // 2 - 100, HEIGHT // 2 + 30, 200, 50)
    rules_rect = pygame.Rect(WIDTH // 2 - 100, HEIGHT // 2 + 90, 200, 50)
    credits_rect = pygame.Rect(WIDTH // 2 - 100, HEIGHT // 2 + 150, 200, 50)
    quit_rect = pygame.Rect(WIDTH // 2 - 100, HEIGHT // 2 + 210, 200, 50)

    while menu_running:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                # Este é o único ponto onde sys.exit() é chamado no fluxo normal do jogo
                pygame.quit()
                sys.exit() 
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                if play_rect.collidepoint(mouse_pos):
                    result = game_loop(False)
                    if result == 'quit':
                        pygame.quit()
                        sys.exit()
                    pygame.event.clear() # Limpa eventos para evitar cliques acidentais
                    pygame.time.wait(200) # Pequeno atraso para estabilização
                elif play_comp_rect.collidepoint(mouse_pos):
                    result = game_loop(True)
                    if result == 'quit':
                        pygame.quit()
                        sys.exit()
                    pygame.event.clear()
                    ...
                    pygame.quit()
                    sys.exit() # Este botão ainda fecha o jogo imediatamente
        ...
        # Chama create_button para desenhar os botões (sem passar ações, pois elas são tratadas acima)
        create_button("Jogar (2 Jogadores)", play_rect, DARK_GREEN, LIGHT_GREEN, WHITE)
        create_button("Jogar vs Computador", play_comp_rect, DARK_GREEN, LIGHT_GREEN, WHITE)
        create_button("Regras", rules_rect, BLUE, LIGHT_RED, WHITE)
        create_button("Créditos", credits_rect, BLUE, LIGHT_RED, WHITE)
        create_button("Sair", quit_rect, RED, LIGHT_RED, WHITE)

        pygame.display.update()
        clock.tick(15) # Menor FPS para o menu
